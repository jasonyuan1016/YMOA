...
$(function () {
    ...
    // 判断输入时间是否大于当前时间
    jQuery.validator.addMethod("greaterDate", function (value, element) {
        var date = new Date();
        return value == "" || date < new Date(value);
    }, "请输入大于当前时间");
    // 判断文件格式
    jQuery.validator.addMethod("isFile", function (value, element) {
        return value == "" || /.(jpg|png|gif|dwg|pdf|gif|doc|docx|xlsx|xls|txt|rar|zip|arj|cad|accdb|pptx|rtf|pub)$/i.test(value);
    }, "您选中的文件后缀类型不正确");
    // 判断文件大小
    jQuery.validator.addMethod("isFileSize", function (value, element) {
        if (value == null) {
            return true;
        }
        var files = $(element).prop('files');
        for (var i = 0; i < files.length; i++) {
            if (((files[i].size).toFixed(2))>= (50 * 1024 * 1024)) {
                return false;
            }
        }
        return true;
